...
<template>
  <nav class="topnav">
    ...
    <div class="navlinks sans">
      ...
      <button class="modebtn" @click="setTheme(store.theme === 'dark' ? 'light' : 'dark')">
        {{ store.theme === 'dark' ? 'LIGHT MODE' : 'DARK MODE' }}
      </button>
      <span v-if="shortWallet" class="wallet">{{ shortWallet }}</span>
...
.modebtn{background:none;border:none;font-size:10.5px;letter-spacing:2px;color:var(--ink2);padding:4px 0 3px;border-bottom:2px solid var(--bronze);transition:color .25s;font-family:Verdana,'Segoe UI',Arial,sans-serif;cursor:pointer}
.modebtn:hover{color:var(--bronze-hi)}
